...
# <a name="how-to-collaborate-using-visual-studio-code"></a>Gewusst wie: zusammenarbeiten mit Visual Studio Code
...
- **Linux**: 64-Bit Ubuntu Desktop 16.04 und höher, Fedora Workstation 27 und höher, CentOS 7
...
5. **Linux**: Wenn eine Benachrichtigung zum Installieren fehlender Bibliotheken angezeigt wird:
...
**Klicken Sie** auf das Element "Live Share" Statusleiste, oder drücken Sie **STRG + UMSCHALT + p/cmd + UMSCHALT + p** , und wählen Sie den Befehl "Live Share: Anmelden mit Browser" aus.
...
> **Linux-Benutzer:** Sie werden möglicherweise zur Eingabe eines Benutzercodes aufgefordert, wenn Sie eine ältere Version von Live Share (v 0.3.295 oder niedriger) verwenden. Aktualisieren Sie auf die neueste Version der Erweiterung, oder klicken Sie nach Ihrer Anmeldung auf den Link „Gibt es Probleme?“, um den Code anzuzeigen. Weitere Informationen finden Sie [im Verlauf des Artikels](#sign-in-using-a-user-code).

#

> Erweiterter **Tipp:** Mit den Einstellungen für `liveshare.account` und `liveshare.accountProvider` können Sie auswählen, welches Konto für die automatische Anmeldung verwendet werden soll, falls Sie über zwischengespeicherte Anmelde Informationen für mehrere Konten verfügen.
...
1. Drücken Sie **STRG + UMSCHALT + p/cmd + UMSCHALT + p** , und führen Sie den Befehl "Live Share: Anmelden mit Benutzercode" aus.
...
4. **Starten Sie eine Zusammenarbeitssitzung:**
    ...
    **Klicken Sie** nun einfach auf das Status leisten Element "Live Share", oder drücken Sie **STRG + UMSCHALT + p/cmd + UMSCHALT + p** , und wählen Sie "Live Share: eine Zusammenarbeits Sitzung starten (Freigabe)" aus.
...
6. **Senden Sie den Link an eine andere Person:**
    ...
    > **Sicherheitstipp:** Möchten Sie die Sicherheitsauswirkungen einiger Features von Live Share verstehen? Dann lesen Sie den Artikel [Sicherheit](../reference/security.md).
...
Klicken Sie zum Starten eines Teilnehmers (als Host oder Gast) in der Liste der Teilnehmer in der Live Share Explorer-Ansicht oder der Registerkarte Benutzer definiert auf seinen Namen. Der Kreis neben seinem Namen wird ausgefüllt, um anzugeben, dass Sie diesen folgen.
...
Eine schnelle Möglichkeit, um festzustellen, wer sich in der Zusammenarbeits Sitzung befindet, besteht darin, die Liste der Teilnehmer in der Live Share Explorer-Ansicht oder der Registerkarte Benutzer definiert In den Ansichten werden alle Teilnehmer in Ihrer Sitzung angezeigt.
...
Sie können auch **STRG + UMSCHALT + p/cmd + UMSCHALT + p** drücken und den Befehl "Live Share: Listen Teilnehmer" auswählen oder auf das StatusBar-Element **Klicken** , das die Anzahl der Teilnehmer in Ihrer Sitzung anzeigt.

![Screenshot: Benutzer in der Sitzung](../media/vscode-user-status.png)
...
| Host: Freigabe wird ausgeführt | ![Zustand „Wird freigegeben...“ in Visual Studio Code](../media/vscode-status-sharing.png)| Eine Zusammenarbeitssitzung wird gestartet, und Inhalte werden in Kürze freigegeben. |
| Host: Freigabe | ![Zustand „Freigabe aktiv“ in Visual Studio Code ](../media/vscode-status-active.png)| Eine aktive Zusammenarbeitssitzung ist vorhanden, und Inhalte werden freigegeben. |
| Host: Schreib geschützter Freigabe | ![Zustand „Schreibgeschützt freigeben“ in Visual Studio Code](../media/vscode-status-sharing-read-only.png)| Eine schreibgeschützte Zusammenarbeitssitzung wird freigegeben. |
| Gast: Sitzung beitreten | ![Zustand „Beitritt zur Sitzung“ in Visual Studio Code](../media/vscode-status-joining.png)| Ein Teilnehmer tritt einer vorhandenen Zusammenarbeitssitzung bei. |
| Gast: verknüpft | ![Zustand „Beigetreten“ in Visual Studio Code](../media/vscode-status-active.png) | Ein Teilnehmer ist einer aktiven Zusammenarbeitssitzung beigetreten und empfängt freigegebene Inhalte. |
| Gast: schreibgeschützt | ![Zustand „Schreibgeschützt beigetreten“ in Visual Studio Code](../media/vscode-status-joined-read-only.png) | Ein Teilnehmer ist einer aktiven schreibgeschützten Zusammenarbeitssitzung beigetreten. |
...
- [Schnellstart: beitreten zur ersten Sitzung](../quickstart/share.md)
